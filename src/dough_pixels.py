...
upper_dough = np.array([100, 75, 255])   # 上限
...
def count_dough_pixels(image_path):
    ...
    # 白色麵糰的HSV範圍, 改成全域變數，讓 adjust_color_range() 可以使用
    # lower_dough = np.array([0, 0, 180])      # 下限
    # upper_dough = np.array([100, 75, 255])   # 上限
...
def adjust_color_range(image_path):
    ...
    cv2.createTrackbar('H_min', 'HSV Adjustment', lower_dough[0], 179, nothing)
    cv2.createTrackbar('S_min', 'HSV Adjustment', lower_dough[1], 255, nothing)
    cv2.createTrackbar('V_min', 'HSV Adjustment', lower_dough[2], 255, nothing)
    cv2.createTrackbar('H_max', 'HSV Adjustment', upper_dough[0], 179, nothing)
    cv2.createTrackbar('S_max', 'HSV Adjustment', upper_dough[1], 255, nothing)
    cv2.createTrackbar('V_max', 'HSV Adjustment', upper_dough[2], 255, nothing)
